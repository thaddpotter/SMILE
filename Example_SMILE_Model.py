...
def Light_curve_final(m1, m2, rstar, ecc, phi, omega, alpha, gamma, g, Periods, ti):
    ...
    threads = 16  # threads ran at the same time
    A = np.empty(len(u))
    callable=interp1d(times,pstar)
    ...
    @njit(fastmath=True)
    def pre_bottom(r, pstar, gamma):
        return r*(1-gamma*((1 - (3 / 2) * ((pstar ** 2 - r ** 2) / pstar ** 2) ** 0.5)))
    ...
    def function(variables,gamma):
        ...
        for i, time in enumerate(times):
            bottom_res, bottom_err = integrate.quad(pre_bottom, 0, pstar[i], args=(pstar[i], gamma))
